...
def refine(input_path: str, output_path: str) -> dict:
    ...
    c_resampled = geometry.resample(c, n=700)
    print(f"[contour] resampled: {len(c_resampled)} pts")

    c_smooth = geometry.smooth(c_resampled, kernel=3, mode="wrap")
    ...
    for s in strokes_raw:
        ...
        if len(s) >= 7:
            s = geometry.smooth_stroke(s, kernel=3)
