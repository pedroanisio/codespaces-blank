...
export default function AIFightFK() {
  ...
  const anim = useRef({
    anglesA:ACTIONS.guard.joints, anglesB:ACTIONS.guard.joints,
    targetA:ACTIONS.guard.joints, targetB:ACTIONS.guard.joints,
    fromA:ACTIONS.guard.joints, fromB:ACTIONS.guard.joints,
    blend:1, flashAlpha:0, flashSide:null
  });
  ...
  useEffect(()=>{
    ...
    function tick(ts) {
      ...
      if (st.blend < 1) {
        st.blend = Math.min(1, st.blend + dt * 2.5);
        const t = smoothstep(st.blend);
        st.anglesA = lerpAngles(st.fromA, st.targetA, t);
        st.anglesB = lerpAngles(st.fromB, st.targetB, t);
      }

      rA = solveFK(rootA, 0, st.anglesA);
      rB = solveFK(rootB, Math.PI, st.anglesB);
      updateFighter(fA, rA.landmarks);
      updateFighter(fB, rB.landmarks);
      ...
      camera.position.y = 1.2 + Math.sin(ts*.002)*.003;
  ...
  const doExchange = useCallback(async()=>{
    ...
    const st=anim.current;
    st.fromA={...st.anglesA};st.fromB={...st.anglesB};
    st.targetA=r.jointsA;st.targetB=r.jointsB;st.blend=0;
    if(r.flash){st.flashAlpha=1;st.flashSide=r.flash;}
    ...
    setTimeout(()=>{
      const st2=anim.current;st2.fromA={...st2.anglesA};st2.fromB={...st2.anglesB};
      st2.targetA=ACTIONS.guard.joints;st2.targetB=ACTIONS.guard.joints;st2.blend=0;
  ...
  const reset=useCallback(()=>{
    ...
    const st=anim.current;st.anglesA=ACTIONS.guard.joints;st.anglesB=ACTIONS.guard.joints;st.targetA=ACTIONS.guard.joints;st.targetB=ACTIONS.guard.joints;st.fromA=ACTIONS.guard.joints;st.fromB=ACTIONS.guard.joints;st.blend=1;st.flashAlpha=0;
    addLog("Reset","ok");
  },[addLog]);
