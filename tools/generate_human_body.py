...
def _csg_flat_bone(l: float, w: float, d: float, name: str) -> dict:
    ...
    # Sternum: elongated plate with manubrium
    if "Sternum" in name:
        return _op("union", [
            _prim("box", halfExtents=vec3(round(w / 2, 2), round(l * 0.35, 2), round(d / 2, 2)),
                  position=vec3(0, 0, 0)),
            _prim("box", halfExtents=vec3(round(w * 0.4, 2), round(l * 0.12, 2), round(d * 0.55, 2)),
                  position=vec3(0, round(l * 0.42, 1), 0)),
            _prim("cylinder", radiusTop=round(d * 0.3, 2), radiusBottom=round(d * 0.15, 2),
...
def gen_bone_geometries(r: Reg) -> list[dict]:
    """Generate parametric CSG geometry for all 206 bones.

    Each bone gets a CSG tree tailored to its classification and anatomy.
    All geometry is in bone-local frame (Y-up = long axis, origin = bone center).
    Trees are validated against CSG_MAX_NODE_COUNT (64) and CSG_MAX_DEPTH (8).

    Reference: Requicha, ACM Comput. Surv. 12(4):437-464, 1980 (CSG theory).
    """
    _CSG_BUILDERS = {
        "long": _csg_long_bone,
        "flat": _csg_flat_bone,
...
def generate_human_body(variation: int = 0) -> dict:
    ...
    bone_geometries = gen_bone_geometries(r)
